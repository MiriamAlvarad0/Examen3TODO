...
const TodoApp = () => {
  ...
  const [todoToEdit, setTodoToEdit] = React.useState(null); // Nuevo estado para editar tarea
  ...
  const deleteTodoWithConfirmation = (todoId) => {
    showModal(
  ...
  const editTodo = (todoId) => {
    ...
    setTextInput(todoToEdit.task); // Rellenar el campo de texto con la tarea seleccionada
    setModalVisible(true); // Abrir el modal para editar
  };

  const updateTodo = () => {
    if (textInput.trim() === '') {
      showModal('Por favor, ingrese un texto válido');
    } else {
      const updatedTodos = todos.map((item) => {
        if (item.id === todoToEdit.id) {
          return { ...item, task: textInput.trim(), dateTime: new Date().toLocaleString() };
        }
        return item;
      });
      setTodos(updatedTodos);
  ...
  const ListItem = ({ todo }) => (
    <TaskCard>
      ...
      <Icon
        ...
        onPress={() => editTodo(todo.id)}  // Agregar la opción de editar
      />
      <Icon
        name="delete"
        size={20}
        color={COLORS.danger}
        onPress={() => deleteTodoWithConfirmation(todo.id)}
      />
    </TaskCard>
  );
  ...
  return (
    <Container>
      ...
      >
        <ModalBackground>
          <ModalBox>
            <ModalText>{todoToEdit ? 'Editar tarea' : modalMessage}</ModalText>
            {todoToEdit && (
              <TextInput
                value={textInput}
            ...
            <ButtonRow>
              ...
              {onConfirm && !todoToEdit && (
                <ModalButton onPress={onConfirm}>
                  <ModalButtonText>Aceptar</ModalButtonText>
                </ModalButton>
              )}
              {todoToEdit && (
                <ModalButton onPress={updateTodo}>  {/* Botón para confirmar la edición */}
                  <ModalButtonText>Actualizar</ModalButtonText>
                </ModalButton>
              )}
            </ButtonRow>
          </ModalBox>
